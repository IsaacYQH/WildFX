...
def has_sidechain_cycle(sidechain_graph: Dict[int, int], source: int, target: int) -> bool:
    """
...
def build_sidechain_graph(fx_chains) -> Dict[int, int]:
    ...
    for chain_idx, chain in enumerate(fx_chains):
        for fx_setting in chain.FxChain:
            if hasattr(fx_setting, 'sidechain_input') and fx_setting.sidechain_input is not None:
