...
from .sidechain_utils import build_sidechain_graph, detect_sidechain_cycles


@lru_cache(maxsize=None)
def _load_preset_data(preset_path: str) -> dict:
    """Load immutable preset metadata once per worker process."""
    with open(preset_path, "r", encoding="utf-8") as preset_file:
        return json.load(preset_file)


def find_closest(sorted_list: List[float], target: Optional[float]) -> Optional[float]:
    """
...
class FXSetting:
    ...
    n_inputs: Optional[int] = (
        None  # Will be populated/validated from preset JSON during load
    )
    n_outputs: Optional[int] = (
        None  # Will be populated/validated from preset JSON during load
    )
    sidechain_input: Optional[int] = (
        None  # Index of the FxChain providing the sidechain signal (None if disabled)
    )
...
class ChainDefinition:
    ...
    # prev_chains removed
    next_chains: Dict[int, float] = field(
        default_factory=dict
    )  # Indices of chains this one feeds into with weights/gains
...
class Project:
    ...
    def __init__(
        self,
        FxChains: List[ChainDefinition],
        input_audios: List[InputAudio],
        output_audio: str = None,
        customized: bool = True,
        plugin_presets_dir: str = PLUGIN_PRESETS_DIR,
    ):
        ...
        if validation_errors:
            error_msg = (
                "Validation failed during Project initialization:\n"
    ...
    def _parse_project_data(args):
        ...
        if not isinstance(project_data, dict):
            raise TypeError(
                f"Expected item {proj_idx} in the list of {yaml_path} to be a dictionary."
            )

        # --- Parse FxChains ---
        chain_defs_data = project_data.get("FxChains", [])
        chain_defs = []
        customized_flag = project_data.get(
            "customized", True
        ...
        for chain_idx, chain_data in enumerate(chain_defs_data):
            if not isinstance(chain_data, dict):
                raise TypeError(
                    f"Project {proj_idx}, Chain {chain_idx}: Expected items in 'FxChains' to be dictionaries."
                )
            ...
            if not isinstance(fx_settings_data, list):
                raise TypeError(
                    f"Project {proj_idx}, Chain {chain_idx}: Expected 'FxChain' within a chain definition to be a list."
                )
            ...
            for fx_idx, fx_data in enumerate(fx_settings_data):
                if not isinstance(fx_data, dict):
                    raise TypeError(
                        f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx}: Expected items in 'FxChain' list to be dictionaries."
                    )
                ...
                if fx_name is None:
                    raise ValueError(
                        f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx}: Missing 'fx_name'."
                    )
                if fx_type is None:
                    raise ValueError(
                        f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx} ('{fx_name}'): Missing 'fx_type'."
                    )
                if fx_type not in ALLOWED_FX_TYPES:
                    raise ValueError(
                        f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx} ('{fx_name}'): Invalid 'fx_type' '{fx_type}'. Allowed types: {ALLOWED_FX_TYPES}"
                    )
                ...
                try:
                    preset_data = _load_preset_data(os.path.abspath(fx_path))
                except FileNotFoundError:
                    raise FileNotFoundError(
                        f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx}: Preset definition file not found for '{fx_name}' at {fx_path} (needed for param validation)."
                    )
                except json.JSONDecodeError as e:
                    raise ValueError(
                        f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx}: Error decoding JSON from preset file {fx_path}: {e}"
                    )
                except Exception as e:
                    raise ValueError(
                        f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx}: Error processing preset file {fx_path} for '{fx_name}': {e}"
                    )

                if preset_data.get("fx_name") != fx_name:
                    raise ValueError(
                        f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx}: Preset file {fx_path} does not match the fx_name '{fx_name}' in YAML."
                    )
                valid_params_dict = preset_data.get("valid_params")
                if valid_params_dict is None or not isinstance(valid_params_dict, dict):
                    raise ValueError(
                        f"Preset file {fx_path} for {fx_name} is missing "
                        "'valid_params' dictionary."
                    )
                valid_params_values = list(valid_params_dict.values())
                ...
                if customized_flag:
                    ...
                    if params_from_yaml:
                        if not isinstance(params_from_yaml, list):
                            raise TypeError(
                                f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx} ('{fx_name}'): 'params' must be a list or null."
                            )
                        if len(valid_params_values) != len(params_from_yaml):
                            raise ValueError(
                                f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx} ('{fx_name}'): Invalid number of params. Expected {len(valid_params_values)}, got {len(params_from_yaml)}."
                            )

                        # Check/adjust params against valid ranges/values
                        final_params = []
                        for i, (param_val, valid_value_list) in enumerate(
                            zip(params_from_yaml, valid_params_values)
                        ):
                            if param_val is None:  # Allow null for default
                                ...
                                if not math.isfinite(float(param_val)):
                                    raise ValueError(
                                        f"Project {proj_idx}, Chain {chain_idx}, "
                                        f"FX {fx_idx}: Parameter {i} must be finite."
                                    )
                                if (
                                    not valid_value_list
                                ):  # If valid_value_list is empty, the final param is None
                                    closest_valid = None
                                else:
                                    # Assuming valid_value_list is sorted list of allowed discrete values
                                    closest_valid = find_closest(
                                        sorted(valid_value_list), float(param_val)
                                    )
                                    if (
                                        closest_valid is None
                                    ):  # Should not happen if valid_value_list is not empty
                                        raise ValueError(
                                            f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx} ('{fx_name}'): Could not find closest value for param {i} ({param_val}) in {fx_path}."
                                        )
                                final_params.append(closest_valid)
                            else:
                                raise TypeError(
                                    f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx} ('{fx_name}'): Parameter {i} must be a number or null, got {type(param_val)}."
                                )
                        ...
                        if (
                            preset_index_yaml is None
                            or not isinstance(preset_index_yaml, int)
                            or preset_index_yaml < 0
                        ):
                            raise ValueError(
                                f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx} ('{fx_name}'): Missing or invalid 'preset_index' when 'params' are not provided."
                            )

                        presets_list = preset_data.get("presets")
                        if presets_list is None or not isinstance(presets_list, list):
                            raise ValueError(
                                f"Preset file {fx_path} for {fx_name} is missing 'presets' list."
                            )
                        if not (0 <= preset_index_yaml < len(presets_list)):
                            raise IndexError(
                                f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx} ('{fx_name}'): 'preset_index' {preset_index_yaml} is out of bounds for presets in {fx_path} (found {len(presets_list)} presets)."
                            )

                        selected_preset = presets_list[preset_index_yaml]
                        if len(selected_preset) != len(valid_params_values):
                            raise ValueError(
                                f"Preset {preset_index_yaml} for {fx_name} has "
                                f"{len(selected_preset)} values; expected "
                                f"{len(valid_params_values)}."
                            )
                        final_params = [
                            value if valid_values else None
                            for value, valid_values in zip(
                                selected_preset, valid_params_values
                            )
                        ]
                    ...
                    if (
                        n_inputs_preset is None
                        or not isinstance(n_inputs_preset, int)
                        or n_inputs_preset < 1
                    ):
                    ...
                    if (
                        n_outputs_preset is None
                        or not isinstance(n_outputs_preset, int)
                        or n_outputs_preset < 1
                    ):
                    ...
                    if preset_load_error:
                        ...
                        raise ValueError(
                            f"Project {proj_idx}, Chain {chain_idx}, FX {fx_idx} ('{fx_name}'): {preset_load_error}"
                        )
                    ...
                    if n_inputs_yaml is not None:
                        if not isinstance(n_inputs_yaml, int) or n_inputs_yaml < 1:
                            print(
                                f"Warning: Project {proj_idx}, Chain {chain_idx}, FX {fx_idx} ('{fx_name}'): 'n_inputs' provided in YAML ({n_inputs_yaml}) is invalid. Must be a positive integer. Using value from preset file."
                            )
                        if n_inputs_yaml != n_inputs_preset:
                            print(
                                f"Warning: Project {proj_idx}, Chain {chain_idx}, FX {fx_idx} ('{fx_name}'): 'n_inputs' in YAML ({n_inputs_yaml}) does not match the value in preset file {fx_path} ({n_inputs_preset}). Using value from preset file."
                            )
                    ...
                    if n_outputs_yaml is not None:
                        if not isinstance(n_outputs_yaml, int) or n_outputs_yaml < 1:
                            print(
                                f"Warning: Project {proj_idx}, Chain {chain_idx}, FX {fx_idx} ('{fx_name}'): 'n_outputs' provided in YAML ({n_outputs_yaml}) is invalid. Must be a positive integer. Using value from preset file."
                            )
                        if n_outputs_yaml != n_outputs_preset:
                            print(
                                f"Warning: Project {proj_idx}, Chain {chain_idx}, FX {fx_idx} ('{fx_name}'): 'n_outputs' in YAML ({n_outputs_yaml}) does not match the value in preset file {fx_path} ({n_outputs_preset}). Using value from preset file."
                            )
                else:
            ...
            next_chains_data = chain_data.get(
                "next_chains", {}
            )  # Default to empty dict
            if not isinstance(next_chains_data, dict):
                raise TypeError(
                    f"Project {proj_idx}, Chain {chain_idx}: Expected 'next_chains' to be a dictionary, got {type(next_chains_data)}."
                )
            ...
            for next_idx_str, weight in next_chains_data.items():
                try:
                    ...
                    raise ValueError(
                        f"Project {proj_idx}, Chain {chain_idx}: 'next_chains' contains non-integer key '{next_idx_str}'."
                    )
                if (
                    ...
                    raise ValueError(
                        f"Project {proj_idx}, Chain {chain_idx}: 'next_chains' weight for key {next_idx} must be a non-negative number, got {weight}."
                    )
        ...
        for audio_idx, audio_data in enumerate(input_audios_data):
            if not isinstance(audio_data, dict):
                raise TypeError(
                    f"Project {proj_idx}, InputAudio {audio_idx}: Expected items in 'input_audios' to be dictionaries."
                )
            ...
            if audio_path is None:
                raise ValueError(
                    f"Project {proj_idx}, InputAudio {audio_idx}: Missing 'audio_path'."
                )
            if audio_type is None:
                raise ValueError(
                    f"Project {proj_idx}, InputAudio {audio_idx}: Missing 'audio_type'."
                )
            if input_fx_chain is None or not isinstance(input_fx_chain, int):
                raise ValueError(
                    f"Project {proj_idx}, InputAudio {audio_idx}: Missing or invalid 'input_FxChain'."
                )
            input_audios.append(
                InputAudio(
        ...
        try:
            ...
            raise ValueError(
                f"Validation failed for project {proj_idx} defined in {yaml_path}:\n{validation_error}"
            )
    ...
    def load_from_yaml(
        ...
        if not isinstance(data_list, list):
            raise TypeError(
                f"Expected top-level structure in {yaml_path} to be a list."
            )

        # Prepare arguments for each worker process
        tasks = [
            (proj_idx, project_data, plugin_presets_dir, yaml_path)
            for proj_idx, project_data in enumerate(data_list)
        ]

        if num_cores == 1 or len(tasks) <= 1:
            projects = [Project._parse_project_data(task) for task in tasks]
        else:
            worker_count = min(num_cores or multiprocessing.cpu_count(), len(tasks))
        ...
        for project in projects:
            project_data = {
                "FxChains": [
                    {
                        "FxChain": [
                            fx_setting.__dict__ for fx_setting in chain_def.FxChain
                        ],
                ...
                "input_audios": [
                    input_audio.__dict__ for input_audio in project.input_audios
                ],
    ...
    def validate(self) -> List[str]:
        ...
        if len(input_chain_indices) != len(self.input_audios):
            errors.append(
                "Input Rule Violation: each input audio must use a distinct "
                "input_FxChain; merge stems through separate input chains."
            )
        ...
        for audio_idx, audio in enumerate(self.input_audios):
            if not audio.audio_path or not isinstance(audio.audio_path, str):
                errors.append(
                    f"InputAudio {audio_idx}: audio_path must be a non-empty string."
                )
                # For now, there is no check for audio_type
            if audio.input_FxChain not in valid_chain_indices:
                errors.append(
                    f"InputAudio {audio_idx}: input_FxChain index {audio.input_FxChain} is invalid."
                )
        ...
        for current_chain_idx, chain in enumerate(self.FxChains):
            ...
            if not isinstance(chain.FxChain, list):
                errors.append(f"Chain {current_chain_idx}: FxChain is not a list.")
            if not isinstance(chain.next_chains, dict):
                errors.append(
                    f"Chain {current_chain_idx}: next_chains must be a dictionary."
                )
            ...
            for next_idx, weight in chain.next_chains.items():
                ...
                if not isinstance(next_idx, int):
                    errors.append(
                        f"Chain {current_chain_idx}: next_chains contains non-integer key '{next_idx}'."
                    )
                    continue
                if next_idx not in valid_chain_indices:
                    errors.append(
                        f"Chain {current_chain_idx}: next_chains contains invalid index {next_idx}."
                    )
                    continue
                if (
                    not isinstance(weight, (int, float))
                    ...
                    errors.append(
                        f"Chain {current_chain_idx}: Weight {weight} for next_chain {next_idx} must be non-negative."
                    )
                    continue
        ...
        for idx in range(num_chains):
            if in_degree[idx] == 0:
                ...
                if idx in input_chain_indices:
                    ...
                    errors.append(
                        f"Input Rule Violation: Chain {idx} is a start node (no predecessors) but not listed in input_audios."
                    )
        ...
        for idx in input_chain_indices:
            if in_degree[idx] != 0:
                errors.append(
                    f"Input Rule Violation: Chain {idx} listed in input_audios but has predecessors (indices: {predecessors[idx]})."
                )
        ...
        while queue:
            ...
            if nodes_in_layer == 0:
                break  # Should not happen if queue is not empty, but safe
            ...
            for _ in range(nodes_in_layer):
                ...
                if not isinstance(chain.FxChain, list):
                    errors.append(
                        f"Chain {current_chain_idx}: FxChain must be a list, got {type(chain.FxChain)}."
                    )
                    ...
                    for fx_idx, fx_setting in enumerate(chain.FxChain):
                        if not isinstance(fx_setting, FXSetting):
                            errors.append(
                                f"Chain {current_chain_idx}, FxChain item {fx_idx}: Expected FXSetting object, got {type(fx_setting)}."
                            )
                        ...
                        # a. Check fx_name
                        if not fx_setting.fx_name or not isinstance(
                            fx_setting.fx_name, str
                        ):
                            errors.append(
                                f"Chain {current_chain_idx}, FX {fx_idx}: 'fx_name' must be a non-empty string, got '{fx_setting.fx_name}'."
                            )

                        # b. Check fx_type
                        if not fx_setting.fx_type or not isinstance(
                            fx_setting.fx_type, str
                        ):
                            errors.append(
                                f"Chain {current_chain_idx}, FX {fx_idx} ('{fx_setting.fx_name}'): 'fx_type' must be a non-empty string."
                            )
                        elif fx_setting.fx_type.lower() not in ALLOWED_FX_TYPES:
                            errors.append(
                                f"Chain {current_chain_idx}, FX {fx_idx} ('{fx_setting.fx_name}'): Invalid 'fx_type' '{fx_setting.fx_type}'. Allowed types: {ALLOWED_FX_TYPES}"
                            )
                        ...
                        if fx_setting.preset_index is None:
                            if fx_setting.params is None:
                                errors.append(
                                    f"Chain {current_chain_idx}, FX {fx_idx} ('{fx_setting.fx_name}'): 'preset_index' is null, but 'params' is also null. At least one must be provided."
                                )
                        elif (
                            not isinstance(fx_setting.preset_index, int)
                            or fx_setting.preset_index < 0
                        ):
                            errors.append(
                                f"Chain {current_chain_idx}, FX {fx_idx} ('{fx_setting.fx_name}'): 'preset_index' must be a non-negative integer or null, got {fx_setting.preset_index}."
                            )

                        # d. Check params structure and content
                        if not isinstance(fx_setting.params, list):
                            errors.append(
                                f"Chain {current_chain_idx}, FX {fx_idx} ('{fx_setting.fx_name}'): 'params' must be a list, got {type(fx_setting.params)}."
                            )
                        else:
                            # Check values are valid types (float, int, or None)
                            for param_idx, param_value in enumerate(fx_setting.params):
                                if param_value is not None and not isinstance(
                                    param_value, (float, int)
                                ):
                                    errors.append(
                                        f"Chain {current_chain_idx}, FX {fx_idx}: Parameter {param_idx} must be a number or None, got {type(param_value)}"
                                    )
                                elif param_value is not None and not math.isfinite(
                                    float(param_value)
                                ):
                                    errors.append(
                                        f"Chain {current_chain_idx}, FX {fx_idx}: "
                                        f"Parameter {param_idx} must be finite."
                                    )
                        ...
                        if (
                            not isinstance(fx_setting.n_inputs, int)
                            or fx_setting.n_inputs < 1
                        ):
                            errors.append(
                                f"Chain {current_chain_idx}, FX {fx_idx} ('{fx_setting.fx_name}'): n_inputs must be a positive integer, got {fx_setting.n_inputs}"
                            )
                        if (
                            not isinstance(fx_setting.n_outputs, int)
                            or fx_setting.n_outputs < 1
                        ):
                            errors.append(
                                f"Chain {current_chain_idx}, FX {fx_idx} ('{fx_setting.fx_name}'): n_outputs must be a positive integer, got {fx_setting.n_outputs}"
                            )
                        ...
                        fx_path = os.path.join(
                            self.plugin_presets_dir, f"{safe_name}.json"
                        )
                        try:
                            preset_data = _load_preset_data(os.path.abspath(fx_path))
                            if preset_data.get("n_inputs") != fx_setting.n_inputs:
                                errors.append(
                                    f"Chain {current_chain_idx}, FX {fx_idx} ('{fx_setting.fx_name}'): n_inputs from preset ({preset_data.get('n_inputs')}) does not match n_inputs in YAML ({fx_setting.n_inputs})."
                                )
                            if preset_data.get("n_outputs") != fx_setting.n_outputs:
                                errors.append(
                                    f"Chain {current_chain_idx}, FX {fx_idx} ('{fx_setting.fx_name}'): n_outputs from preset ({preset_data.get('n_outputs')}) does not match n_outputs in YAML ({fx_setting.n_outputs})."
                                )
                        except FileNotFoundError:
                            errors.append(
                                f"I/O Channel Validation: Preset definition file not found for '{fx_setting.fx_name}' at {fx_path} (needed for param validation)."
                            )
                        except json.JSONDecodeError as e:
                            errors.append(
                                f"I/O Channel Validation: Error decoding JSON from preset file {fx_path}: {e}"
                            )
                        except Exception as e:
                            errors.append(
                                f"I/O Channel Validation: Error processing preset file {fx_path} for '{fx_setting.fx_name}': {e}"
                            )
                        ...
                        if fx_setting.sidechain_input is not None:
                            ...
                            # REAPER exposes one stereo control bus (channels 3/4)
                            # for a track, so distinct controls for multiple FX in
                            # one chain cannot be represented unambiguously.
                            if sidechain_count > 1:
                                errors.append(
                                    f"Chain {current_chain_idx}: Contains multiple "
                                    "sidechain-enabled plugins. Only one sidechain "
                                    "input is allowed per chain."
                                )

                            sc_source_idx = fx_setting.sidechain_input
                            if not isinstance(sc_source_idx, int):
                                errors.append(
                                    f"Chain {current_chain_idx}, FX {fx_idx}: sidechain_input must be integer index or null."
                                )
                            elif (
                                fx_setting.n_inputs <= 1
                            ):  # Check n_inputs for sidechain capability
                                errors.append(
                                    f"Chain {current_chain_idx}, FX {fx_idx} ('{fx_setting.fx_name}'): sidechain_input is set, but plugin's n_inputs ({fx_setting.n_inputs}) is not > 1."
                                )
                            elif sc_source_idx not in valid_chain_indices:
                                errors.append(
                                    f"Chain {current_chain_idx}, FX {fx_idx}: sidechain_input index {sc_source_idx} is invalid."
                                )
                            elif (
                                sc_source_idx not in init_queue
                            ):  # Check if the source is in the current processing layer, so that REAPER can set track send correctly
                                errors.append(
                                    f"Chain {current_chain_idx}, FX {fx_idx} ('{fx_setting.fx_name}'): sidechain_input index {sc_source_idx} is not in the current layer ({current_layer}). Sidechain sources must be in the same layer as consumers."
                                )
                            else:
                                ...
                                if (
                                    ...
                                    errors.append(
                                        f"Chain {current_chain_idx}, FX {fx_idx}: Sidechain input cannot come from a splitter output (chain {sc_source_idx})."
                                    )
                        ...
                        if fx_setting.fx_type.lower() == "splitter":
                            ...
                            if not is_last_fx:
                                errors.append(
                                    f"Splitter Rule Violation: Chain {current_chain_idx}, FX {fx_idx} ('{fx_setting.fx_name}') is a Splitter but not the last FX."
                                )
                            else:  # Is last FX and is Splitter
                                if (
                                    fx_setting.n_outputs <= fx_setting.n_inputs
                                ):  # Splitter must have more outputs than inputs
                                    errors.append(
                                        f"Chain {current_chain_idx}, Last FX ('{fx_setting.fx_name}'): Splitter must have n_outputs > n_inputs."
                                    )
                                else:
                                    expected_next_chains = (
                                        fx_setting.n_outputs / fx_setting.n_inputs
                                    )  # Calculate expected next_chains count based on the Splitter's outputs vs inputs
                                    if len(chain.next_chains) > expected_next_chains:
                                        errors.append(
                                            f"Chain {current_chain_idx}: Splitter requires {expected_next_chains} next_chains (n_outputs/n_inputs), found {len(chain.next_chains)}."
                                        )
                                    elif len(chain.next_chains) < expected_next_chains:
                                        print(
                                            f"Warning: Chain {current_chain_idx}: Splitter has too few next_chains ({len(chain.next_chains)}), expected {expected_next_chains}. If this is intentional, ignore this warning."
                                        )

                # 2. Check next_chains type and content
                if not isinstance(chain.next_chains, dict):
                    errors.append(
                        f"Chain {current_chain_idx}: next_chains must be a dictionary, got {type(chain.next_chains)}."
                    )
                else:
                    for next_idx, weight in chain.next_chains.items():
                        if not isinstance(next_idx, int):
                            errors.append(
                                f"Chain {current_chain_idx}: next_chains key '{next_idx}' must be an integer."
                            )
                        elif next_idx not in valid_chain_indices:
                            errors.append(
                                f"Chain {current_chain_idx}: next_chains key {next_idx} is an invalid chain index."
                            )

                        if not isinstance(weight, (float, int)):
                            errors.append(
                                f"Chain {current_chain_idx}: next_chains weight for key {next_idx} must be a number, got {type(weight)}."
                            )
                        elif not math.isfinite(float(weight)):
                            errors.append(
                                f"Chain {current_chain_idx}: next_chains weight for key {next_idx} must be finite, got {weight}."
                            )
                        elif weight < 0:
                            errors.append(
                                f"Chain {current_chain_idx}: next_chains weight for key {next_idx} must be non-negative, got {weight}."
                            )

                # Post-FX loop checks for the chain
                is_last_fx_splitter = (
                    chain.FxChain and chain.FxChain[-1].fx_type == "splitter"
                )
                # Check count based on the number of keys in next_chains dict
                if len(chain.next_chains) > 1 and not is_last_fx_splitter:
                    errors.append(
                        f"Splitter Rule Violation: Chain {current_chain_idx} has multiple next_chains ({len(chain.next_chains)}) but last FX is not Splitter."
                    )
        ...
        if processed_chains_count != num_chains:
            ...
            errors.append(
                f"Graph contains a cycle or unreachable nodes. Unprocessed/Unreachable chains: {unprocessed}"
            )
        ...
        if cycle_nodes:
            ...
            errors.append(
                f"Sidechain Cycle Detected: Circular dependency involving chains {cycle_list}"
            )

        # 3. Start Node Consistency Check (Re-check after sort)
        if actual_start_nodes != input_chain_indices:
            errors.append(
                f"Input Rule Violation: Mismatch between actual start nodes {actual_start_nodes} and designated input chains {input_chain_indices}."
            )

        # 4. Single Output Node Check (Nodes with empty next_chains dict)
        end_nodes = [
            idx for idx in range(num_chains) if not self.FxChains[idx].next_chains
        ]
        if len(end_nodes) == 0:
            errors.append(
                "Output Rule Violation: No end node found (chain with empty next_chains)."
            )
        elif len(end_nodes) > 1:
            errors.append(
                f"Output Rule Violation: Multiple end nodes found: {end_nodes}. Expected exactly one."
            )
            ...
            if final_chain.FxChain and final_chain.FxChain[-1].fx_type == "splitter":
                errors.append(
                    f"Splitter Rule Violation: Final output chain {final_chain_index} cannot end with a Splitter."
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        output_yaml_file = (
            "/workspaces/WildFX/proj_metadata/metadata_saved.yaml"  # Corrected path
        )
