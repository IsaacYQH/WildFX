...
def choose_random_plugin_parameters(
    valid_params: dict, rng: random.Random | None = None
) -> dict:
    """Select one value for each adjustable plugin parameter.

    Args:
        valid_params: Parameter names mapped to their allowed values.
        rng: Optional local random-number generator.

    Returns:
        Parameter names mapped to deterministic random choices. Parameters with no
        allowed values are represented by ``None``.
    """
    rng = rng or random
    return {
        name: rng.choice(valid_values) if valid_values else None
...
def generate_params_task(args: tuple) -> dict:
    """Generate one deterministic parameter set for a pool worker."""
    index, valid_params_arg, seed = args
    params_choice = choose_random_plugin_parameters(
        valid_params_arg, random.Random(seed + index)
    )
    return params_choice


# Helper function for parallel audio loading ---
def load_process_audio_task(args: tuple) -> tuple | None:
    """Load, validate, normalize, and flatten one rendered audio file.

    Silent and non-finite renders are rejected because they do not provide useful
    features for clustering and usually indicate a failed plug-in render.
    """
    ...
    try:
        ...
        if sr != target_sample_rate:
            return None
        if audio_output.size == 0 or not np.isfinite(audio_output).all():
            return None
        audio_output = audio_output.T

        peak = float(np.max(np.abs(audio_output)))
        if peak <= 1e-8:
            return None
        ...
        if audio_output_norm.ndim > 1 and audio_output_norm.shape[0] > 1:
            audio_flat = np.mean(audio_output_norm, axis=0)
        else:
            audio_flat = np.reshape(audio_output_norm, (-1))

        return index, audio_flat
    except Exception as exc:
        print(f"Error loading/processing {output_path} in worker: {exc}")
        return None
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate audio effect plugin presets using parameter clustering"
    )
    ...
    if not args.no_cluster_validation and args.input_audio is None:
        parser.error("--input-audio is required unless --no-cluster-validation is used")
    if args.cpu_cores < 1:
        parser.error("--cpu-cores must be at least 1")
    if args.batch_size < 1:
        parser.error("--batch-size must be at least 1")
    if args.num_presets is not None and args.num_presets < 1:
        parser.error("--num-presets must be at least 1")
    if args.num_generations is not None and args.num_generations < 1:
        parser.error("--num-generations must be at least 1")
    if (
        args.num_presets is not None
        and args.num_generations is not None
        and args.num_presets > args.num_generations
    ):
        parser.error("--num-presets cannot exceed --num-generations")
            ...
            with open(args.plugin_list, "r") as f:
                ...
                for row in reader:
                    if len(row) >= 2:
                        ...
                        if plugin_name.lower() in {"name", "plugin", "plugin_name"}:
                            continue
                        if plugin_name and plugin_type:
                            if plugin_type in ALLOWED_FX_TYPES:
                                ...
                                parser.error(
                                    f"Plugin type '{plugin_type}' for '{plugin_name}' "
                                    "is not in ALLOWED_FX_TYPES"
                                )
        except Exception as e:
            parser.error(f"Error reading plugin list file: {e}")
    ...
    if args.plugin_name:
        for plugin_name, plugin_type in args.plugin_name:
            ...
            if plugin_type in ALLOWED_FX_TYPES:
                ...
                parser.error(
                    f"Plugin type '{plugin_type}' for '{plugin_name}' is not in "
                    "ALLOWED_FX_TYPES"
                )
        ...
        try:
            with open(args.detected_plugin_list, "r") as f:
                ...
                for row in reader:
                    ...
                    if len(row) < 2:
                        continue
                    first_column = row[0].strip().lower()
                    second_column = row[1].strip()
                    if first_column in {"index", "id"} or second_column.lower() in {
                        "name",
                        "plugin name",
                    }:
                        continue
                    if second_column:
                        available_plugins.add(second_column)
            ...
            if unavailable_plugins:
                ...
                for plugin in unavailable_plugins:
                    print(f"  - {plugin}")
                print("These requested plugins cannot be processed.")
                processing_failures.extend(unavailable_plugins)

        except Exception as e:
            print(
                f"Error reading detected plugin list {args.detected_plugin_list}: {e}"
            )
            print("Continuing without validating plugins...")
    else:
        print(
            f"Warning: Detected plugin list file {args.detected_plugin_list} not found."
        )
        print("Continuing without validating plugins...")
    ...
    if not plugins_to_process:
        parser.error(
            "No plugins specified. Use --plugin-name, --plugin-list, --use-reduced-set, or --use-full-set"
        )
    ...
    atexit.register(shutil.rmtree, tmp_output_dir, True)
    ...
    if not args.no_cluster_validation:
        ...
        try:
            sample_rate = sf.info(audio_input_path).samplerate
        ...
        if use_cuda and num_gpus > 1:
            ...
            mfcc_transform = nn.DataParallel(
                mfcc_transform, device_ids=list(range(num_gpus))
            )
    ...
    for plugin_name in plugins_to_process:
        ...
        if args.skip_existing and os.path.exists(preset_filepath):
        ...
        try:
            ...
            plugin_metadata = fx_get_metadata(
                project, plugin_name, plugins_to_process[plugin_name]
            )
            ...
            print(
                f"Obtained plugin_metadata: {len(valid_params)} parameters, {n_inputs} input channels and {n_outputs}."
            )
        except Exception as e:
            print(f"Error getting plugin_metadata for {plugin_name} via REAPER: {e}")
            print("Skipping this plugin.")
            processing_failures.append(plugin_name)
        ...
        if num_valid_params == 0:
            print(
                f"Skipping {plugin_name} - No adjustable parameters found after filtering."
            )
            processing_failures.append(plugin_name)
            continue
        ...
        if args.num_presets:
            ...
            if calculated_presets > MAX_PRESETS_FROM_PARAMS:
                print(
                    f"Warning: Calculated presets ({calculated_presets}) exceeds cap ({MAX_PRESETS_FROM_PARAMS}). Using {MAX_PRESETS_FROM_PARAMS}."
                )
        ...
        if args.num_generations:
            ...
            num_generations = max(
                num_presets * 5, 100
            )  # Ensure at least 100 generations
        print(
            f"Targeting {num_generations} parameter sets to find {num_presets} presets for {plugin_name} ({num_valid_params} adjustable params)..."
        )

        # Check if num_presets exceeds num_generations after calculation
        if num_presets > num_generations:
            print(
                f"Warning: Number of presets ({num_presets}) exceeds number of generations ({num_generations}). Reducing num_presets to {num_generations}."
            )
        ...
        preset_parameters = []
        generation_count = (
            num_presets if args.no_cluster_validation else num_generations
        )
        generation_tasks = [
            (i, valid_params, args.seed) for i in range(generation_count)
        ]

        print(f"Generating parameters using {cpu} cores...")
        try:
            with multiprocessing.Pool(processes=cpu) as pool:
                preset_parameters = list(
                    tqdm(
                        pool.imap(generate_params_task, generation_tasks),
        ...
        if not args.no_cluster_validation:
            FxChain_list = [
                [{"fx_name": plugin_name, "params": list(params.values())}]
                for params in preset_parameters
                if params is not None
            ]
            tmp_output_paths = [
                os.path.join(tmp_output_dir, f"{safe_instance_name}_{i}.wav")
                for i in range(len(FxChain_list))
            ]
            batch_render_fx(
                project,
                tmp_output_dir,
                [audio_input_path] * len(FxChain_list),
                tmp_output_paths,
                FxChain_list,
                gain_list=[1.0] * len(FxChain_list),
                batch_size=cpu,
                render_tail_seconds=(
                    3.0
                    if plugins_to_process[plugin_name] in {"delay", "echo", "reverb"}
                    else 0.0
                ),
            )
            ...
            print(
                "Loading and normalizing rendered audio from temporary directory (in parallel)..."
            )
            load_tasks = [
                (i, path, sample_rate) for i, path in enumerate(tmp_output_paths)
            ]
            results = []
            ...
            try:
                with multiprocessing.Pool(
                    processes=cpu
                ) as pool:  # Reuse cpu count or define separate for loading
                    results = list(
                        tqdm(
                            pool.imap_unordered(load_process_audio_task, load_tasks),
            ...
            if not flattened_audio_for_mfcc:
                print(
                    f"Error: No audio files were successfully rendered or loaded for {plugin_name}. Skipping clustering."
                )
                processing_failures.append(plugin_name)
                continue  # Skip to next plugin

            print(
                f"Successfully loaded {len(flattened_audio_for_mfcc)} rendered audio files for feature extraction."
            )
            # Filter preset_parameters to only include those corresponding to loaded audio
            filtered_preset_parameters = [
                preset_parameters[i] for i in successfully_loaded_indices
            ]
            ...
            try:
                ...
                for i in tqdm(
                    range(0, num_samples, batch_size), desc="MFCC Mini-Batches"
                ):
                    # Get mini-batch
                    ...
                    features_mini_batch = mfccs_mini_batch.reshape(
                        mfccs_mini_batch.size(0), -1
                    )
                ...
                )  # This is now your feature matrix features
            ...
            if features.shape[0] == 0:  # Check if feature extraction yielded results
                print(
                    f"Error: No features extracted for {plugin_name}. Skipping clustering."
                )
                processing_failures.append(plugin_name)
                continue
            ...
            if features.shape[0] < num_presets:
                print(
                    f"Warning: Number of generated features ({features.shape[0]}) is less than target presets ({num_presets}). Reducing clusters."
                )
                num_presets = features.shape[
                    0
                ]  # Cannot have more clusters than samples

            if num_presets <= 0:
                print(
                    "Error: No presets to generate (num_presets is zero or negative). Skipping clustering."
                )
                processing_failures.append(plugin_name)
                continue

            kmeans = KMeans(
                n_clusters=num_presets,
            ...
            for preset_index in range(num_presets):
                ...
                if len(cluster_indices) > 0:
                    ...
                    preset_params = list(
                        filtered_preset_parameters[cluster_index].values()
                    )
                    pruned_preset_parameters.append(preset_params)
            print(
                f"Generated {len(pruned_preset_parameters)} presets for {plugin_name}."
            )
            ...
            plugin_metadata["presets"] = [
                list(params.values())
                for params in preset_parameters[:num_presets]
                if params is not None
            ]
            print(
                f"Generated {len(plugin_metadata['presets'])} presets for {plugin_name} without clustering."
            )

        # Save the plugin metadata to a JSON file
        json_filepath = os.path.join(plugin_preset_dir, f"{safe_instance_name}.json")

        json_temp_filepath = f"{json_filepath}.tmp"
        with open(json_temp_filepath, "w") as f:
            json.dump(plugin_metadata, f, indent=4)
        os.replace(json_temp_filepath, json_filepath)
        print(f"Saved presets to {json_filepath}")

    # Clean up the temporary directory when done
    shutil.rmtree(tmp_output_dir, ignore_errors=True)
    if processing_failures:
        failed_names = ", ".join(sorted(set(processing_failures)))
        raise SystemExit(f"Preset generation failed for: {failed_names}")
